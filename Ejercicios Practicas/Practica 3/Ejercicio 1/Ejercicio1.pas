...
begin
  ...
  writeln('9. Dar de Baja a Empleado.');
  writeln('10. Salir');
  writeln('---------------------------------------');
  writeln(' ');
end;
...
//REALIZAR BAJA EMPLEADO.
procedure realizarBajaEmpleado(var a:tArch);
var
  regM,aux:empleado;
  encontre: boolean;
  numEmp,posAux:integer;
begin
  ...
  while not EOF(a) and not encontre do begin
    read(a, regM); //Leo un empleado
    
    if (regM.numero = numEmp)then begin
      encontre:= true;
      posAux:= filePos(a) - 1; //Me guardo la posicion del registro a borrar.
      seek(a, fileSize(a) - 1); //Posiciono el puntero en el final del archivo.
      read(a, aux); //Leo el ultimo registro en el archivo.
      seek(a, posAux); //Me posiciono en el registro a borrar.
      write(a, aux); //borro el registro, pisandolo con el ultimo.
      seek(a, fileSize(a)- 1); //Me posiciono en el ultimo registro.
      truncate(a); //borro el ultimo registro del archivo.
    end;
  end;
  
  if not encontre then writeln('No se encontro el numero de empleado ingresado.')
  else writeln('Empleado borrado Exitosamente!.');
